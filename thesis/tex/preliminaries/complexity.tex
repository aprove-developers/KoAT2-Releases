...
\begin{remark}[Time Complexity \cite{koat}]
...
Therefore, if only the information is given, that for an input state $\valuation \in \Valuation$ it holds that $-m \leq \valuation \leq m$, the time complexity $\text{rc}'(m)$ of \cite{koat} and the new time complexity $\text{rc}(-m,m)$ have the same precision.
...
\begin{example}[Comparison of Time Complexity Definitions]
  ...
  \input{graphs/motivational_example_complexity}
  ...
  Therefore, the new time complexity $\text{rc}(\lstate, \ustate)$ is $1 + 4$, which is significantly closer to the real time complexity than the time complexity of \cite{koat} $\text{rc}'(m) = 9$.
\end{example}
