...
\begin{definition}[Program] 
  ...
  Let $\mathbb{Z}[\VSet]_\bot = \mathbb{Z}[\VSet] \cup \braced{\bot}$ be the polynomial ring over the variables $\VSet$ and the undefined value $\bot$.
  Let $\AtomSet = \braced{p \leq 0 \mid \exists p \in \mathbb{Z}[\VSet]}$ be the set of atoms with the usual evaluation of the formula $p \leq 0$. 
  Let $\TSet = \LSet \times (\VSet \rightarrow \mathbb{Z}[\VSet]) \times 2^\AtomSet \times \LSet$ be the set of all transitions of the program.
  ...
  Let $c: \TSet \rightarrow \Valuation \rightarrow \mathbb{Z}_\bot$ be a function which maps each transition to its costs with a specific state.
...
For example $P=(\braced{a,b,c},\braced{(a,\text{id},\text{true},b),(a,\text{id},\text{true},c)})$ is a valid program, although it is possible to both move to $b$ and $c$ from location $a$.
