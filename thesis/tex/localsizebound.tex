...
\begin{definition}[Scaled Sum]
  ...
  Then, we denote with $s^\sqcap_\rv$ the scaling factor $s$, with $e^\sqcap_\rv$ the constant $e$ and with the sets $P_{\rv,1}^\sqcap, P_{\rv,2}^\sqcap, N_{\rv,1}^\sqcap, N_{\rv,2}^\sqcap$ the four variable sets $P_1$, $P_2$, $N_1$ and $N_2$.
  ...
  Then, we denote with $s^\sqcup_\rv$ the scaling factor $s$, with $e^\sqcup_\rv$ the constant $e$ and with the sets $P_{\rv,1}^\sqcup, P_{\rv,2}^\sqcup, N_{\rv,1}^\sqcup, N_{\rv,2}^\sqcup$ the four variable sets $P_1$, $P_2$, $N_1$ and $N_2$.
\end{definition}
...
\begin{example}[Upper Scaled Sum]
  ...
  For transition $t_3$, the update is $\update(x) = 2 \cdot x + 2 \cdot y + 4$.
  ...
  For transition $t_4$, the update is $\update(x) = 2 \cdot x + 3 \cdot y$.
  ...
  Note that this is not a problem in an update $\update(x) = 3 \cdot x + 3 \cdot y$, since the factors are equal.
  Only if the factors are not equal, further logic is necessary.
  ...
  For transition $t_5$, the update is $\update(x) = -2 \cdot x - 3 \cdot y$.
