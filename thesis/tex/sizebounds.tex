...
This is the factor $\maximum{\abs{V_\alpha} \mid \alpha \in \SCC_t}$, which was not yet mentioned, but is part of the \textbf{transition scaling factor} $s^\sqcap_t$.
...
The idea of the factor $\maximum{\abs{V_\alpha} \mid \alpha \in \SCC_t}$ is, that we can overapproximate the effect of $\update(y)$ with $2 \cdot \max(y,z)$.
