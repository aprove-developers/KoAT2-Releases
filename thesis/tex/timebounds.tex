...
This trivial choice requires us to find a ranking function, that satisfies the non-increasing constraint for all transitions of the program.
...
\begin{algorithmic}[1]
  ...
  \ForAll{$i$ from $0$ to $\abs{\TSet}-\abs{\TSet_b}$}
    \ForAll{$t$ in $\TSet \setminus \TSet_b$}
      \ForAll{$\TSet_< \subseteq (\TSet \setminus \braced{t})$ with $\abs{\TSet_\geq} = i$}
