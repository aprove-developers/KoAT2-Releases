...
Because a time bound can only be positive, we apply to the result $\apprsubst{\Pol(\location)}{\LSize(r)}{\USize(r)}$ a $\maxO{\dot}$ operator.
...
Then, $\sum_{\location \in \mathcal{E}_{\TSet'}} \sum_{\pret \in \TSet_\location} \UTime(\pret) \cdot \maxO{\apprsubst{\Pol(\location)}{\LSize(\pret)}{\USize(\pret)}}$ is a time bound for $t \in \TSet'_>$ depending on the variables from the start.
...
\[ \apprsubst{\Pol(\location)}{\LSize(t_0)}{\USize(t_0)} = \USize(t_0, x) - \LSize(t_0, y) = x - y \]
...
Therefore for each $t \in \TSet'_>$ we end up with $\UTime'(t) = \UTime(t_0) \cdot \maxO{\apprsubst{\Pol(\location_1)}{\LSize(t_0)}{\USize(t_0)}}$.
...
Therefore, we end up with $\UTime'(t) = \maxO{\apprsubst{\Pol(\location_2)}{\LSize(t_2)}{\USize(t_2)}}$.

Note that it is possible to select arbitrary subsets $\TSet' \subseteq \TSet$.
A non-connected subset $\TSet' = \braced{t_1, t_3, t_4}$ could be chosen, but would lead to a bound $\UTime'(t) = \maxO{\apprsubst{\Pol(\location_1)}{\LSize(t_0)}{\USize(t_0)}} + \maxO{\apprsubst{\Pol(\location_2)}{\LSize(t_2)}{\USize(t_2)}}$.
