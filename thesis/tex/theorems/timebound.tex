\begin{theorem}[TimeBounds]
  ...
  Let $\Pol: \LSet \rightarrow \BoundSet_a(\PVSet)$ be an \textbf{affine} PRF for $\TSet'$. \\
  For $t \in \TSet'_>$ let
  \[ \UTime'(t) = \sum_{\location \in \mathcal{E}_{\TSet'}} \sum_{\pret \in \TSet_\location} \UTime(\pret) \cdot \maxO{\apprsubst{\Pol(\location)}{\LSize(\pret)}{\USize(\pret)}} \]
