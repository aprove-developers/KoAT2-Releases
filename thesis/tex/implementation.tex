...
For a program $\Program = ((\LSet, \TSet), \location_0, \VSet, c)$ the algorithm needs an initial time and size bound prior to the execution.
...
Therefore, it is sound to use the initial time bound $\UTime$ with $\UTime(t) = c(t)$ for all transitions which does not occur in an SCC of the program $\Program$ and $\UTime(t) = \infty$ for all transitions which does occur in an SCC of the program $\Program$.
