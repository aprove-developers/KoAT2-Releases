...
\begin{tikzpicture}[->,>=stealth',auto,node distance=5cm,
  ...
  \path[every node/.style={font=\sffamily\small}]
    (0) edge[aligned edge] node[above=0.2cm] {$t_0$} node[below=0.2cm] {$\update = \emph{id}$\\$\guard = \emptyset$} (1)
    (0) edge[aligned edge] node[above=0.2cm] {$t_1$} node[below=0.2cm] {$\update = \emph{id}$\\$\guard = \emptyset$} (2)
    ;
\end{tikzpicture}
