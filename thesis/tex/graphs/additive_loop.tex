...
\begin{tikzpicture}[->,>=stealth',auto,node distance=5cm,
  ...
  \path[every node/.style={font=\sffamily\small}]
    ...
    (1) edge[aligned edge, loop above] node[left=0.2cm] {$t_1$} node[right=0.2cm] {$\update(x) = x - 1$\\$\update(y) = y + z$\\$\update(z) = z$\\$\guard = \braced{x > 0}$} (1)
    ;
\end{tikzpicture}
...
\caption{Program which adds a constant value multiple times in a loop}
