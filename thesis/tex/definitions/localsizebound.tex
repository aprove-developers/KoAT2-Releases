...
Therefore we are looking for a model of the \todo{Proper definition of eval for formulas?}{following formula}.
...
\begin{definition}[Trivial upper local size bound]
  ...
  Let $s, e \in \mathbb{N}$ be the multiplication of the absolute values of all integers occuring in any upper update $\uupdate(v)$ or any guard atom in $\guard$.
...
\begin{algorithmic}[1]
  ...
  \ForAll {variables $v \in \VSet$}
    ...
    \If {$\bigwedge \guard \Rightarrow \uupdate(v) \leq b$ is not valid}
