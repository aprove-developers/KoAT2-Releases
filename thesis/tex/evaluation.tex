...
The following definition introduces a projection to overapproximate the actual asymptotic complexity of a bound.
...
We can prove that for every bound $b \in \BoundSet(\VSet)$ the resulting asymptotic complexity $\complexity(b)$ is a valid overapproximation of the actual complexity.

\input{theorems/asymptotic_complexity}

Note that with this definition a bound $x-x$ is projected to a complexity $\complexity(x-x) = \landau(n)$.
