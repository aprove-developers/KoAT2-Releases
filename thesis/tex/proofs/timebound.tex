...
\begin{IEEEeqnarray*}{rCl}
  \sum_{\location \in \mathcal{E}_{\TSet'}} \sum_{\pret \in \TSet_l} \exacteval{\UTime(\pret)}{\valuation_0} \cdot \maxO{\exacteval{\apprsubst{\Pol(\location)}{\LSize(\pret)}{\USize(\pret)}}{\valuation_0}} \\
  \geq \timeboundterm
\end{IEEEeqnarray*}
...
\begin{IEEEeqnarray*}{rCl}
  \sum_{\location \in \mathcal{E}_{\TSet'}} \sum_{\pret \in \TSet_l} \exacteval{\UTime(\pret)}{\valuation_0} \cdot \maxO{\exacteval{\apprsubst{\Pol(\location)}{\LSize(\pret)}{\USize(\pret)}}{\valuation_0}} \geq k
\end{IEEEeqnarray*}
...
\todo{Show with Complexities from PRFs proof from old koat}{We can show that $\maxO{\exacteval{\Pol(\location_i)}{\actstate_i}}$ is an upper bound on the number of times the transition $t$ is used in this sequence.}
