...
\end{equation}
Instead of proving (\ref{eq:sizebound_target}) directly, we prove a simplified hypothesis, which implies the target (\ref{eq:sizebound_target}).
...
With those properties, we can conclude that it is sufficient to prove the hypothesis (\ref{eq:sizebound_hypothesis}).
...
Now we define a bound for the value of the variables computed up to the transition $\actt$.
...
Similar, for the lower case we have
{\allowdisplaybreaks
\begin{IEEEeqnarray*}{rClr}
  & & \ueval{d^\sqcup}{\lstate}{\ustate} \\
  ...
  & \geq & \scale^\sqcup_\actt \cdot \left( \ueval{\dpre{\sqcup}}{\lstate}{\ustate} + \ueval{\effect^\sqcup_\actt}{\lstate}{\ustate} \right) & \text{since } \ueval{\effect^\sqcup_\actt}{\lstate}{\ustate} \geq 0 \\
  ...
  & \geq & \scale^\sqcup_\actt \cdot \left( \ueval{\dpre{\sqcup}}{\lstate}{\ustate} + \ueval{\effect^\sqcup_\actrv}{\lstate}{\ustate} \right) & \text{since } \actrv \in \SCC_\actt \\
  & = & \left( \maximum{s^\sqcup_{\rv} \mid \rv \in \SCC_\actt} \cdot
    \maximum{\abs{\VSet_\rv} \mid \rv \in \SCC_\actt} \right) \cdot
    \left( \ueval{\dpre{\sqcup}}{\lstate}{\ustate} + \ueval{\effect^\sqcup_\actrv}{\lstate}{\ustate} \right) & \text{substitute } \scale^\sqcup_\actt \\
