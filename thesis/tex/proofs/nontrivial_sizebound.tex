...
Furthermore, the transition effects are always positive (e.g. $\ueval{\effect^\sqcap_\actt}{\lstate}{\ustate} \geq 0$ and $\ueval{\effect^\sqcup_\actt}{\lstate}{\ustate} \geq 0$) and the starting value $\start \geq 0$ is always positive.
...
For the upper case, since $(\pret, v) \in \pre(\actt, v) \setminus \SCC$, we have $\ueval{\start}{\lstate}{\ustate} \geq \ueval{\USize(\pret, v)}{\lstate}{\ustate}$.
...
For the lower case, we have $\ueval{\start}{\lstate}{\ustate} \geq -\leval{\LSize(\pret, v)}{\lstate}{\ustate}$.
...
\todo{Can we prove this?}{For all variables $v \in \VSet^-_\rv$ we know that $\ueval{\dpre{\sqcap}}{\lstate}{\ustate} \geq -\prestate(v)$ and $\ueval{\dpre{\sqcup}}{\lstate}{\ustate} \geq \prestate(v)$.}
...
\begin{equation} \label{eq:d_lower_reduction}
  \begin{split} 
  & \sum_{v \in \VSet_\actrv} \ueval{\dpre{\sqcup}}{\lstate}{\ustate} \\
  = & \sum_{v \in \VSet^+_\actrv} \ueval{\dpre{\sqcup}}{\lstate}{\ustate} + \sum_{v \in \VSet^-_\actrv} \ueval{\dpre{\sqcup}}{\lstate}{\ustate} \\
  \geq & \sum_{v \in \VSet^+_\actrv} \maxO{-\prestate(v)} + \sum_{v \in \VSet^-_\actrv} \maxO{\prestate(v)} (\ref{eq:sizebound_hypothesis}) \\
  \end{split}      
\end{equation}
