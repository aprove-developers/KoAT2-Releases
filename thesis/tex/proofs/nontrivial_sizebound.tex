...
\begin{IEEEeqnarray*}{rClr}
  & & \eval{d^\square}{m} \\
  & = & \prod_{t \in \TSet} (s^\square_t)^{k_t} \cdot \left( e + \sum_{t \in \TSet} \left( k_t \cdot \eval{\effect^\square_t}{m} \right) \right) \\
  & = & \left( s^\square_{\hat{t}} \cdot (s^\square_{\hat{t}})^{k_{\hat{t}} - 1} \cdot \prod_{t \in \TSet \setminus \braced{\hat{t}}} (s^\square_t)^{k_t} \right) \cdot \\
    && \left( e + \eval{\effect^\square_{\hat{t}}}{m} + \left( k_{\hat{t}} - 1 \right) \cdot \eval{\effect^\square_{\hat{t}}}{m} + \sum_{t \in \TSet \setminus \braced{\hat{t}}} \left( k_t \cdot \eval{\effect^\square_t}{m} \right) \right)
    & (\ref{eq:scaling_factor_reduction}), (\ref{eq:input_reduction}) \\
  & = & s^\square_{\hat{t}} \cdot \left( \dpre{\square} + \left( (s^\square_{\hat{t}})^{k_{\hat{t}} - 1} \cdot \prod_{t \in \TSet \setminus \braced{\hat{t}}} (s^\square_t)^{k_t} \right) \cdot \eval{\effect^\square_{\hat{t}}}{m} \right) & (\ref{eq:induction_consequence}) \\
  & \geq & s^\square_{\hat{t}} \cdot \left( \dpre{\square} + \eval{\effect^\square_{\hat{t}}}{m} \right) & \text{since } \eval{\effect^\square_{\hat{t}}}{m} \geq 0 \\
  & \geq & s^\square_{\hat{t}} \cdot \left( \dpre{\square} + \max \braced{\maxO{\pm e^{\square}_\alpha} + \eval{\effect^\square_\alpha}{m} \mid \alpha \in \SCC_{\hat{t}} } \right) & \text{by definition} \\
  & \geq & s^\square_{\hat{t}} \cdot \left( \dpre{\square} + \maxO{\pm e^{\square}_{\hat{\alpha}}} + \eval{\effect^\square_{\hat{\alpha}}}{m} \right) & \text{since } {\hat{\alpha}} \in \SCC_{\hat{t}} \\
  & = & \maximum{s^\square_{\alpha} \mid \alpha \in \SCC_{\hat{t}}} \cdot \\
    && \maximum{\abs{\pre(\alpha) \cap \SCC} \mid \alpha \in \SCC_{\hat{t}}} \cdot \\
    && \left( \dpre{\square} + \maxO{\pm e^{\square}_{\hat{\alpha}}} + \eval{\effect^\square_{\hat{\alpha}}}{m} \right) & \text{substitute } s^\square_{\hat{t}} \\
  & \geq & s^\square_{\hat{\alpha}} \cdot \abs{\pre(\hat{\alpha}) \cap \SCC} \cdot \left( \dpre{\square} + \maxO{\pm e^\square_{\hat{\alpha}}} + \eval{\effect^\square_{\hat{\alpha}}}{m} \right)
    & \text{since } \hat{\alpha} \in \SCC_{\hat{t}} \\
  & \geq & s^\square_{\hat{\alpha}} \cdot \left( \abs{\pre(\hat{\alpha}) \cap \SCC} \cdot \dpre{\square} + \maxO{\pm e^\square_{\hat{\alpha}}} + \eval{\effect^\square_{\hat{\alpha}}}{m} \right) \\
  & \geq & s^\square_{\hat{\alpha}} \cdot \left( \abs{\pre(\hat{\alpha}) \cap \SCC} \cdot \dpre{\square} \pm e^{\square}_{\hat{\alpha}} + \eval{\effect^\square_{\hat{\alpha}}}{m} \right) \\
  & \geq & s^\square_{\hat{\alpha}} \cdot \left( \sum_{v \in \VSet_{\hat{\alpha}}} \dpre{\square} \pm e^\square_{\hat{\alpha}} + \eval{\effect^\square_{\hat{\alpha}}}{m} \right) \\
