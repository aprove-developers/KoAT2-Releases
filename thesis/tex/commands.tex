...
\newcommand{\apprsubst}[3]{\text{replace me}}
\newcommand{\exactsubst}[2]{\llbracket #1 \rrbracket_{#2}}
\newcommand{\oversubst}[3]{\lceil #1 \rceil^{#3}_{#2}}
\newcommand{\undersubst}[3]{\lfloor #1 \rfloor^{#3}_{#2}}

\newcommand{\appreval}[3]{\text{replace me}}
\newcommand{\exacteval}[2]{\llbracket #1 \rrbracket_{#2}}
\newcommand{\undereval}[3]{\lfloor #1 \rfloor^{#3}_{#2}}
\newcommand{\overeval}[3]{\lceil #1 \rceil^{#3}_{#2}}

\newcommand{\timeboundterm}{
  \sup \braced{ k \in \mathbb{N} \mid \exists \valuation_0, (\location, \valuation): \valuation^\sqcup \leq \valuation_0 \leq \valuation^\sqcap \wedge (\location_0, \valuation_0) (\rightarrow^* \circ \rightarrow_t)^k (\location, \valuation) }
}

\newcommand{\sizeboundterm}{
  \braced{ \valuation(v) \mid \exists \valuation_0, (\location, \valuation): \valuation^\sqcup \leq \valuation_0 \leq \valuation^\sqcap \wedge (\location_0, \valuation_0) (\rightarrow^* \circ \rightarrow_\actt) (\location, \valuation)}
}
...
\newcommand{\localsizeboundterm}{
  \braced{\valuation'(v) \mid \exists (\location, \valuation), (\location', \valuation'): \valuation^\sqcup \leq \valuation \leq \valuation^\sqcap \wedge (\location, \valuation) \rightarrow_t (\location', \valuation')}
}
...
\newcommand{\costcomplexityterm}{
  \braced{ \sum_{0 \leq i \leq k} \exacteval{\cost(t_i)}{\valuation_i} \mid \exists \valuation_0, k \geq 1: 
    \valuation^\sqcup \leq \valuation_0 \leq \valuation^\sqcap \wedge (\location_0, \valuation_0) \rightarrow_{t_0} (\location_1, \valuation_1) \rightarrow_{t_1} \dots \rightarrow_{t_k} (\location_k, \valuation_k) }
}
