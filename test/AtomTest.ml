    ...
    >::: [
           ...
           List.map
             (fun (testname, expected, atom) ->
               testname >:: fun _ -> assert_equal_atom expected (KoatReaders.read_atom atom))
             [
           ...
           >::: List.map
                  (fun (testname, atom) ->
                    testname >:: fun _ ->
                    assert_raises (Koat_lexer.SyntaxError testname) (fun _ -> KoatReaders.read_atom atom))
                  [ ("Unexpected char: =", "x = y") ];
         ]
end
...
module Methods = struct
  let rename str rename_map =
    str |> KoatReaders.read_atom |> fun atom -> Atom.rename atom (RenameMap.from_native rename_map)
  ...
  let tests =
    ...
    >::: [
           ...
           >::: List.map
                  (fun (atom1, atom2) ->
                    atom1 ^ "=~=" ^ atom2 >:: fun _ ->
                    assert_equal_atom (KoatReaders.read_atom atom1) (KoatReaders.read_atom atom2))
                  [
                    ("x < y", "y > x");
                    ("x <= y", "y >= x");
                    ("x <= y", "x - 1 < y");
           ...
           >::: List.map
                  (fun (expected, atom) ->
                    ...
                    assert_equal_varset (VarSet.of_string_list expected)
                      (Atom.vars (KoatReaders.read_atom atom)))
                  [
                    ([ "x" ], " x^3+2*x -1 < x^5 ");
                    ( [ "x"; "y"; "z" ],
           ...
           >::: List.map
                  (fun (expected, atom) ->
                    atom >:: fun _ ->
                    assert_equal_atom (KoatReaders.read_atom expected)
           ...
           >::: List.map
                  (fun (expected, atom) ->
                    atom >:: fun _ -> assert_equal_bool expected (Atom.is_linear (KoatReaders.read_atom atom)))
                  [ (true, "x < y"); (false, "x <= a^2 + b * 3 -6"); (false, "x >= a^2 + b * 3 -6") ];
           "negation"
           >::: List.map
                  (fun (expected, atom) ->
                    atom >:: fun _ ->
                    assert_equal_atom (KoatReaders.read_atom expected) (KoatReaders.read_atom atom |> Atom.neg))
