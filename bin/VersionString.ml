...
let git_version: string =
  [%meta
    ...
    match env_var with
    | None   ->
        let version = read_process "cd $DUNE_SOURCEROOT; git rev-parse --short HEAD" and
            date    = read_process "cd $DUNE_SOURCEROOT; git log -1 --format=%cs"    in
        let version_str =  version^" from "^date in

        let b = Buffer.create 10 in
        String.iter (fun c -> if not (Char.equal c '\n') then Buffer.add_char b c) version_str;
        let str_without_newlines = Buffer.contents b in
        Ppxlib.Parse.expression (Lexing.from_string @@ "\""^str_without_newlines^"\"")

    | Some x ->
        Ppxlib.Parse.expression (Lexing.from_string @@ "\""^x^"\"")
  ]
