...
type params = {
  ...
  var : string; [@default "x"]  (** The variable for which a local size bound should be found. *)
}
...
let run (params : params) =
  ...
  print_string
    (Bound.to_string
       LocalSizeBound.(
         find_bound (Set.inter (Formula.vars guard) (VarSet.singleton var)) var guard 1024
         |> Option.map ~f:Tuple2.first |> option_lsb_as_bound))
