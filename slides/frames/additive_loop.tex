\begin{columns}
  \begin{column}{0.6\textwidth}
    ...
    \begin{tikzpicture}[program]    
      ...
      \path[transitions]
      (0) edge node[above] {\textcolor<6-7,11-12>{blue}{$t_0$}} node[below] {$x \coloneqq x$\\$y \coloneqq y$\\\nodeshow{2-}{\textcolor<2>{blue}{$z \coloneqq z$}}} (1)
      (1) edge[loop above] node[above left] {\textcolor<5,8,13,16>{blue}{$t_1$}} node[above tlabel] {$if(x > y)$\\$x \coloneqq x - 1$\\$y \coloneqq y$\\\nodeshow{2-}{\textcolor<2>{blue}{$z \coloneqq \textcolor<5-6,11>{blue}{z} + \textcolor<7-8,12,13,15>{blue}{y}$}}} node[above right=-1cm and 0.4cm] {\phantom{$\leq 2 \cdot \max(z,y)$}} (1)
      ;
    \end{tikzpicture}
  \end{column}
