...
type strategy = { run_strategy: 'p. run_preprocessor:('p t -> 'p -> 'p MaybeChanged.t) -> 'p t list -> 'p -> 'p }
...
module Make(PM: ProgramTypes.ProgramModules)(CPM: ProgramTypes.ClassicalProgramModules)
           (Eq: sig val eq: (PM.Program.t,CPM.Program.t) Type_equal.t end)= struct
  open PM
  ...
  let transform (t: PM.Program.t t) (subject: PM.Program.t): PM.Program.t MaybeChanged.t =
    ...
    | CutUnsatisfiableTransitions ->
      let module CUT = CutUnsatisfiableTransitions.Make(PM) in CUT.transform_program subject
...
module MakeForClassicalProgramModules(CPM: ProgramTypes.ClassicalProgramModules) = struct
  include Make(CPM)(CPM)(struct let eq = Type_equal.refl end)
end
