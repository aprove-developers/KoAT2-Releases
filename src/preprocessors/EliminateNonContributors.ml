...
module Make(M: ProgramTypes.ProgramModules) = struct
    ...
    let rec eliminate_t_ vars get_update contributors non_contributors =
        let (xs,ys) = Set.fold ~f:(fun (contr,non_contr) y ->
                            if  Set.exists ~f:(fun x -> Polynomial.vars (get_update x |? Polynomial.zero) |> flip Set.mem y) contr then
                                (Set.add contr y, Set.remove non_contr y)
                            else
                                (contr,non_contr)) vars
                            ~init:(contributors, non_contributors) in
    ...
    let rec eliminate_ program contributors non_contributors =
        let (xs,ys) = Base.Set.fold ~f:(fun (xs,ys) (l,t,l') ->
                        Base.Set.fold ~f:(fun (contr,non_contr) y ->
                            if depends y t contr then
                                (Set.add contr y, Set.remove non_contr y)
                            else
                              (contr,non_contr)) ys ~init:(xs,ys))
    ...
    let eliminate_t vars vars_guard get_update remove_non_contributors =
        ...
        Logger.(log logger INFO (fun () -> "EliminateNonContributors", [("init_contr", VarSet.to_string init_contr);("init_non_contributors", VarSet.to_string (Set.diff vars vars_guard))]));
    ...
    let eliminate program =
        ...
        let vars_guard = Base.Set.fold ~f:(fun xs (l,t,l') -> Set.union (Constraint.vars (TransitionLabel.guard t)) xs) (Program.transitions program) ~init:VarSet.empty
        and vars_cost = Base.Set.fold ~f:(fun xs (l,t,l') -> Set.union (Polynomial.vars (TransitionLabel.cost t)) xs) (Program.transitions program) ~init:VarSet.empty in
