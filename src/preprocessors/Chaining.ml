...
let skip_location trans_id_counter location graph =
  ...
  |> Enum.map (fun ((l,t,_), (_,t',l')) -> (l, TransitionLabel.append trans_id_counter t t', l'))
...
let transform_graph trans_id_counter (graph: TransitionGraph.t): TransitionGraph.t MaybeChanged.t =
