...
let skip_location location graph =
  ...
  |> Tuple2.mapn Sequence.of_list
  |> uncurry Sequence.cartesian_product
...
let chain location graph : TransitionGraph.t =
  ...
  |> List.fold ~f:TransitionGraph.remove_edge_e ~init:skipped
...
let transform_graph ?(conservative=false) ?(scc=None) (graph: TransitionGraph.t): TransitionGraph.t MaybeChanged.t =
  let try_chaining location maybe_changed_graph =
    ...
    maybe_changed_graph >>= (fun graph ->
      if (Option.is_none scc || Set.mem (TransitionSet.locations (Option.value_exn scc)) location) && chainable ~conservative graph location then (
