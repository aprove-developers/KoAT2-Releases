...
module Make (M : ProgramTypes.ClassicalProgramModules) = struct
  ...
  let transform_program_ program_ (program : Program.t) =
    ...
    let vars =
      List.fold
        ~f:(fun vars (_, t, _) -> Set.union vars (TransitionLabel.vars t))
        ~init:VarSet.empty (Set.to_list transitions)
    in
    ...
    let apply_guard (guard : Guard.t) (abstract : 'a Apron.Abstract1.t) : 'a Apron.Abstract1.t =
      constraint_to_apron environment guard |> Apron.Abstract1.meet_tcons_array manager abstract
    in

    (* Applies the update to the abstract value. *)
    let apply_update (update : Var.t -> Polynomial.t Option.t) (abstract : 'a Apron.Abstract1.t) :
        'a Apron.Abstract1.t =
      (* A completely unbound (non-deterministic) choice *)
      let any_value = Apron.(Texpr1.cst environment (Coeff.Interval Interval.top)) in
      let assignments =
        vars |> Set.to_array |> Array.map ~f:update
      ...
      |> constraint_from_apron |> constraint_to_apron environment
    ...
    let apply_transition ((l, t, l') : Transition.t) (abstract : 'a Apron.Abstract1.t) : 'a Apron.Abstract1.t
        =
      abstract |> apply_guard (TransitionLabel.guard t) |> apply_update (TransitionLabel.update t)
    in
    ...
    let find_fixpoint (program_abstract : 'a program_abstract) : 'a program_abstract =
      ...
      let worklist : Transition.t Stack.t = Set.to_list transitions |> Stack.of_list in
      ...
      let narrowing_abstract =
        let location_abstract l =
          ...
          |> Sequence.fold ~f:(Apron.Abstract1.join manager)
               ~init:(Apron.Abstract1.bottom manager environment)
        in
        Set.to_list locations
    ...
    let extract_invariant (abstract : 'a Apron.Abstract1.t) : Constraint.t =
      abstract |> Apron.Abstract1.to_tcons_array manager |> constraint_from_apron
    in

    (* Determines for each location its invariant constraint and filters locations, which does not have an additional invariant. *)
    let invariants =
      bottom |> find_fixpoint |> Hashtbl.map ~f:extract_invariant
    ...
    if Hashtbl.is_empty invariants then
      ...
      let add location invariant program =
        Logger.(
          log logger INFO (fun () ->
              ( "add_invariant",
                [ ("location", Location.to_string location); ("invariant", Constraint.to_string invariant) ]
              )));
        ProofOutput.add_str_paragraph_to_proof (fun () ->
            "Found invariant "
            ^ Constraint.to_string ~pretty:true invariant
            ^ " for location " ^ Location.to_string location);
        Program.add_invariant location invariant program
      in
      Hashtbl.fold ~f:(fun ~key ~data -> add key data) invariants ~init:program |> MaybeChanged.changed


  (* TODO Actually, we should check if the new invariant is already implied and only then say, that it is changed. *)
