...
let transform_program program =
  let prob_0_trans =
    ...
    |> Set.filter
         ~f:OurFloat.(equal zero % TransitionLabel.probability % Transition.label)
  in
  ...
  Set.iter
    ...
    prob_0_trans;
  let lift = if Set.is_empty prob_0_trans then MaybeChanged.same else MaybeChanged.changed in
