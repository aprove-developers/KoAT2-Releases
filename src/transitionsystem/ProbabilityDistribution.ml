...
let to_string ?(pretty=false) ?(to_file=false) =
  let poly_to_string =
    if pretty then Polynomial.to_string_pretty else
      if to_file then Polynomial.to_string_to_file else Polynomial.to_string
  in
  function
...
let admissibility_constraint = let zero = Polynomial.zero in function
  | Binomial       (n,_)      -> Guard.Infix.(zero <= n)
  | Hypergeometric (bigN,k,n) -> let bigN = Polynomial.of_constant bigN in
...
let vars = function
  ...
  | Hypergeometric (_,k,n) -> VarSet.union (Polynomial.vars k)  (Polynomial.vars n)
  | Uniform        (p1,p2) -> VarSet.union (Polynomial.vars p1) (Polynomial.vars p2)

let as_guard d v' = let poly_v' = Polynomial.of_var v' in let zero = Polynomial.zero in
  match d with
  | Binomial (n,p)            ->
      if      OurFloat.(equal p zero) then Guard.Infix.(poly_v' = zero)
      else if OurFloat.(equal p one)  then Guard.Infix.(poly_v' = n)
      else                                 Guard.Infix.(zero <= poly_v' && poly_v' <= n)
...
let moment_poly d i =
  if i = 1 then exp_value_poly d
    ...
    | Binomial       (n,p)      -> (match i with
        | 2 -> let n,p = RealPolynomial.of_intpoly n, RealPolynomial.of_constant p in
          RealPolynomial.(n * p * (one - p) + pow n 2 * pow p 2)
        | _ -> failwith @@ Int.to_string i^". moment of binomial distribution not yet implemented."
      )
    | Geometric      a          -> (match i with
    ...
    | Hypergeometric (bigN,k,n) -> ( match i with (* RealPolynomial.(of_intpoly k * of_intpoly n * of_constant OurFloat.(one/of_ourint bigN)) *)
        | 2 -> let expv = exp_value_poly d in let bigN,k,n = OurFloat.of_ourint bigN,RealPolynomial.of_intpoly k,RealPolynomial.of_intpoly n in
          RealPolynomial.(expv + expv * (of_constant OurFloat.(one/bigN) * (of_constant bigN - k)) *
    ...
    | Uniform (a,b) ->
        match i with
          | 2 -> (* Variance is given by ((b - a + 1)² - 1)/12 *)
              let one = RealPolynomial.of_constant OurFloat.one in
              let variance =
                RealPolynomial.mul (RealPolynomial.of_constant @@ OurFloat.div OurFloat.one (OurFloat.of_int 12))
                  (RealPolynomial.sub
                    (RealPolynomial.pow
                        (RealPolynomial.add (RealPolynomial.sub (RealPolynomial.of_intpoly b) (RealPolynomial.of_intpoly a)) one)
                        2)
                    one)
              in
