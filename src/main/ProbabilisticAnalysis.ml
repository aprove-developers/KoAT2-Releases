  ...
  |> Option.map (fun (program, goal) ->
         ...
         |> tap (fun (program, appr) ->
                if params.print_rvg then (
                  GraphPrint.print_rvg (CacheManager.rvg_cache cache) (CacheManager.pre_cache cache) (CacheManager.lsb_cache cache) `Lower ~label:RV.to_id_string ~outdir:output_dir ~file:input_filename program;
                  GraphPrint.print_rvg (CacheManager.rvg_cache cache) (CacheManager.pre_cache cache) (CacheManager.lsb_cache cache) `Upper ~label:RV.to_id_string ~outdir:output_dir ~file:input_filename program
                )
              )
         |> (fun (program, appr) ->
         ...
         |> tap (fun (program, appr) ->
                if params.print_rvg then (
                  GraphPrint.print_rvg (CacheManager.rvg_cache cache) (CacheManager.pre_cache cache) (CacheManager.lsb_cache cache) `Lower ~label:(bounded_rv_to_string (CacheManager.lsb_cache cache) program `Lower appr) ~outdir:output_dir ~file:input_filename program;
                  GraphPrint.print_rvg (CacheManager.rvg_cache cache) (CacheManager.pre_cache cache) (CacheManager.lsb_cache cache) `Upper ~label:(bounded_rv_to_string (CacheManager.lsb_cache cache) program `Upper appr) ~outdir:output_dir ~file:input_filename program;
                )
              )
         |> tap (fun (program, appr) ->
