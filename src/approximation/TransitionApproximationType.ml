  ...
  let add ?(simplifyfunc = identity) bound transition (name, map) =
    let map =
      Map.change map transition
        ~f:
          (Option.some
          % Option.value_map ~f:(simplifyfunc % B.keep_simpler_bound bound) ~default:(simplifyfunc bound))
    in
    Logger.log logger Logger.INFO (fun () ->
        ("add_" ^ name ^ "_bound", [ ("transition", T.to_id_string transition); ("bound", B.to_string bound) ]));
    (name, map)


  let all_bounded appr = Sequence.for_all ~f:(fun t -> not (B.equal (get appr t) B.infinity))

  let to_formatted ?(pretty = false) ?(termination_only = false) transitions t =
    transitions |> List.sort ~compare:T.compare
    |> List.map ~f:(fun trans -> (trans, get t trans))
    |> List.map ~f:(fun (trans, b) ->
           FormattedString.mk_str_line @@ "  " ^ T.ids_to_string ~pretty trans ^ ": "
           ^ B.to_string ~pretty ~termination_only b)
