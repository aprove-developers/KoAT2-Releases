...
let find_bounds ?(mprf_max_depth = 1) ~preprocess ?(cfr = false) ?(time_cfr = 180) ?(inv = false) (program: Program.t) (appr: Approximation.t): Program.t * Approximation.t =
  ...
  |> Analysis.improve rvg_with_sccs ~mprf_max_depth ~preprocess ~cfr ~inv `Time program in
  let appr_cost =
  updated_appr
  |> (fun appr ->
    if program_cfr |> Program.transitions |> TransitionSet.exists (fun t -> not (Polynomial.is_const (Transition.cost t))) then
      Tuple2.second @@ Analysis.improve rvg_with_sccs ~mprf_max_depth ~preprocess ~cfr ~inv `Cost program_cfr appr
    else
      appr
  )
