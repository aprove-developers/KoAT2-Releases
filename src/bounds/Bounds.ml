...
let find_bounds ~preprocess ~local ~cfr ?(time_cfr = 180) ?(twn = false) (program: Program.t) (appr: Approximation.t): Program.t * Approximation.t =
  ...
  |> Analysis.improve rvg_with_sccs ~preprocess ~local ~cfr `Time program in
  let appr_cost =
  updated_appr
  |> (fun appr ->
    if program_cfr |> Program.transitions |> TransitionSet.exists (fun t -> not (Polynomial.is_const (Transition.cost t))) then
      Tuple2.second @@ Analysis.improve rvg_with_sccs ~preprocess ~local ~cfr `Cost program_cfr appr
    else
      appr
  )
