...
let ranking_function_procedure (program : Program.t) (transitions : transitionEnum) =
  ...
  let (table, fresh_coeffs) = generate_ranking_template program locations in
    print_string ("ranking_function_procedure, generated coeffs = "^(String.concat "," (List.map (Var.to_string) fresh_coeffs)));
    ...
    let model = SMTSolver_.get_model (Formula.mk smt_form) in (*(fresh coeffs should be used here)*)
