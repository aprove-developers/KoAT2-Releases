...
(*module Make(P : ProgramTypes.Program) =
  struct*)
    ...
    type t = {
        pol : Program_.Location.t -> Polynomial.t (*This should be a parameter Polynomial, so that it can be used a few times*);
        ...
        let num_of_fresh = List.length b_right in
            let fresh_vars = Var.fresh_id_list num_of_fresh in
                let dual_constr = Constraints_.dualise fresh_vars a_matrix c_left in
                    let cost_constr = Polynomial_.from_coeff_list b_right fresh_vars in
        ...
        let num_vars = (List.length vars) in
            let fresh_coeffs = List.map Polynomial_.from_var (Var.fresh_id_list num_vars) in
                let linear_poly = ParameterPolynomial.from_coeff_list fresh_coeffs vars in
                    let constant = ParameterPolynomial.from_constant (Polynomial_.from_var (List.at (Var.fresh_id_list 1) 1)) in
    ...
    let help_non_increasing (table : (Program_.TransitionGraph.vertex, ParameterPolynomial.t) Hashtbl.t) (trans : Program_.TransitionGraph.E.t) (vars : Var.t list) =
    ...
    let help_strict_decrease (table : (Program_.TransitionGraph.vertex, ParameterPolynomial.t) Hashtbl.t) (trans : Program_.TransitionGraph.E.t) (vars : Var.t list) =
        let trans_label = Program_.TransitionGraph.E.label trans in
            let start_parapoly = Hashtbl.find table (Program_.TransitionGraph.E.src trans) in
                let target_parapoly = Hashtbl.find table (Program_.TransitionGraph.E.dst trans) in
                    let guard = TransitionLabel.guard trans_label in
    ...
    let help_boundedness (table : (Program_.TransitionGraph.vertex, ParameterPolynomial.t) Hashtbl.t) (trans : Program_.TransitionGraph.E.t) (vars : Var.t list) =
        let trans_label = Program_.TransitionGraph.E.label trans in
            let start_parapoly = Hashtbl.find table (Program_.TransitionGraph.E.src trans) in
                let guard = TransitionLabel.guard trans_label in
