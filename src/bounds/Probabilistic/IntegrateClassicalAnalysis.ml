...
module Make (BP : BoundPair.T) = struct
  ...
  module UnliftedBound = UnliftedBounds.UnliftedTimeBound.Make (NonProbOverappr) (BP.ClassBound)
  ...
  let gts_to_nonprob_transs gts =
    ...
    |> NonProbOverappr.TransitionSet.map ~f:prob_trans_to_nonprob_trans


  let get_timebound direction (class_appr, appr) t =
    let prob_trans = nonprob_trans_to_prob_trans t in
    match direction with
    | `ExpTime -> BP.prob_to_class_bound @@ ExpApproximation.timebound appr (Transition.gt prob_trans)
  ...
  let get_sizebound direction (class_appr, appr) t var =
    ...
    | `ExpSize ->
        ...
        BP.prob_to_class_bound @@ ExpApproximation.sizebound appr (gt, Transition.target prob_trans) var
  ...
  let get_timebound_and_unbounded_vars_both_directions program_vars apprs =
    ...
    let is_timebound_finite = BP.ClassBound.is_finite % get_timebound in
    let unbounded_vars t = Set.filter program_vars ~f:(BP.ClassBound.is_infinity % get_sizebound t) in
  ...
  let improve_with_mprfs depth program scc (class_appr, appr) =
    ...
    let find_mprf t =
      ...
      let find_of_depth depth =
        MultiphaseRankingFunction.find_scc `Time class_program is_time_bounded unbounded_vars
          all_trans_non_prob depth nonprob_trans
      in
    ...
    |> MaybeChanged.fold_sequence ~init:appr ~f:(fun appr (gt, mprfs) ->
           ...
           in
           let new_bound =
             List.map ~f:(fun (b, _) -> b) new_bounds_and_proofs
             |> BP.class_to_prob_bound % BP.ClassBound.sum_list
           in
           ProofOutput.add_to_proof_with_format
             FormattedString.(
               fun format ->
                 ...
                 let new_bound =
                   mk_str_line @@ "Obtained bound " ^ BP.ProbBound.to_string ~pretty:true new_bound
                 in
                 let mprf_proofs =
                   List.map
  ...
  type twn_state = {
    bounds_from_twn_loops :
      ( Transition.t,
        BP.ClassBound.t * (Formatter.format -> FormattedString.t),
  ...
  let twn_state_to_string twn_state =
    Printf.sprintf "bounds_from_twn_loops: %s\nremaining_twn_loops: %s\n"
      ...
      |> Util.sequence_to_string ~f:(fun (t, (b, _)) ->
             Transition.to_id_string_pretty t ^ ": " ^ BP.ClassBound.to_string b))
      ...
      |> Util.sequence_to_string ~f:(fun l_w_p ->
             ...
             |> Util.sequence_to_string ~f:NonProbOverappr.Transition.to_id_string_pretty))
  ...
  let initial_twn_state program scc =
    ...
    let module Check_TWN = Check_TWN.Make (BP.ClassBound) (NonProbOverappr.TransitionLabelNonFunctionCall) in
    let all_loops =
  ...
  let compute_timebounds_from_loop_with_proof ?(twnlog = false) ?(unsolvable = false) twn_state apprs
    ...
    if not (Set.is_empty newly_handled_trans) then
      ...
      let compute_proof format =
        FormattedString.(
          compute_proof format
          <> mk_block (ProofOutput.LocalProofOutput.get_proof time_and_size_proofs format))
      in
      if BP.ClassBound.is_finite new_bound then
  ...
  let improve_with_twn_loops ?(twnlog = false) ?(unsolvable = false) twn_state (class_appr, appr)
      unbounded_gts loops_with_proofs =
    ...
    |> Sequence.filter_map ~f:(fun gt ->
           ...
           let b =
             ...
             |> Sequence.map ~f:(fun (_, (b, _)) -> BP.class_to_prob_bound b)
             |> BP.ProbBound.sum
           in
           ProofOutput.add_to_proof_with_format
             FormattedString.(
        ...
        |> Sequence.find_map ~f:(fun (gt, loops_with_proofs) ->
               let all_new_boundable_loops =
                 List.filter
                   ...
                   loops_with_proofs
               in
               let all_new_boundable_trans =
                 handled_transs_of_twn_loops_with_proofs all_new_boundable_loops
               in
               if
