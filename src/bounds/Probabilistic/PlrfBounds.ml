...
module Make = struct
  ...
  let get_timebound direction (class_appr, appr) =
    ...
    | `ClassTime ->
        fun (gt, l) ->
          ...
          |> BP.ClassBound.sum |> BP.class_to_prob_bound
  ...
  let get_combined_bounds_both_directions apprs (gt, l) =
    ...
    if BP.ProbBound.is_finite classtime_bound then
  ...
  let improve_with_plrf program (class_appr, appr) rank =
    ...
    Logger.log logger Logger.DEBUG (fun () ->
        ("improve_with_plrf", [ ("rank", Plrf.to_string rank); ("bound", BP.ProbBound.to_string new_bound) ]));
