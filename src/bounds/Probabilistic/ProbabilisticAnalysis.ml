...
module Make = struct
  ...
  let lift_bounds program program_vars gts apprs : ExpApproximation.t =
    let lift_size_bounds_like view set appr =
      let gt_timebound gt =
        ...
        |> BP.class_to_prob_bound % BP.ClassBound.sum
      in
  ...
  module ClassicAnalysis = Analysis.Make (BP.ClassBound) (NonProbOverappr)
  module CostBounds = CostBounds.Make (BP.ClassBound) (NonProbOverappr)
  ...
  let analyse_refined_scc ~conf apprs_orig program_orig scc_orig program_vars cfr ~refined_program =
    ...
    let updated_apprs_cfr =
      ...
      List.fold_left cfr_sccs_locs
        ~init:(CFR.create_new_apprs program_orig refined_program apprs_orig)
        ~f:update
    in
    ...
    CFR.add_proof_to_global_proof cfr ~refined_program
      ~refined_bound_str:(BP.ProbBound.show_complexity @@ BP.ProbBound.asymptotic_complexity cfr_bound);

    (* Get CFR proof & restore original proof *)
    let cfr_subproof = ProofOutput.get_subproof () in

    if BP.ProbBound.compare_asy org_bound cfr_bound < 1 then (
      Logger.log logger_cfr Logger.INFO (fun () ->
          ( "NOT_IMPROVED",
            [
              ("original bound", BP.ProbBound.to_string ~pretty:true org_bound);
              ("cfr bound", BP.ProbBound.to_string ~pretty:true cfr_bound);
              (CFR.method_name cfr ^ " bound", BP.ProbBound.to_string ~pretty:true cfr_bound);
            ] ));
  ...
  let handle_cfrs ~conf scc_locs program program_vars apprs =
    ...
    let non_linear_transitions =
      Set.filter
        ~f:(not % BP.ProbBound.is_linear % ExpApproximation.timebound apprs.appr % Transition.gt)
        scc_transitions
    in
    if Set.is_empty non_linear_transitions then
      ...
      let refinement_result =
        CFR.iter_cfrs program ~scc_orig:scc_transitions ~transitions_to_refine:non_linear_transitions
          ~compute_timelimit:(fun () ->
            CFR.compute_timeout_time program
              ~infinite_timebound:
                (BP.ProbBound.is_infinity % ExpApproximation.timebound apprs.appr % Transition.gt)
              scc_transitions)
  ...
  let lift_appr_to_exp_costbounds program apprs =
    ...
    Set.fold gts ~init:apprs ~f:(fun apprs gt ->
        let gtcost =
          ...
          |> BP.ProbBound.sum
        in
        let new_bound = BP.ProbBound.mul gtcost (ExpApproximation.timebound apprs.appr gt) in
