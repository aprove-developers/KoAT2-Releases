...
module Make(PM: ProgramTypes.ClassicalProgramModules) = struct
  ...
  let trans_table = Hashtbl.create 10
  ...
  let time_bound conf (l,t,l') scc program appr =
    ...
    let bound =
      ...
      if Option.is_none opt then (
        ...
        let loops_opt = SimpleCycle.find_loops ~relax_loops:conf.relax_loops trans_table (heuristic_for_cycle conf.transformation_type) appr program scc (l,t,l') in
        if Option.is_some loops_opt then
          ...
          List.iter (fun (l,t,l') -> TimeBoundTable.add time_bound_table (l,Hashtbl.find_default trans_table t t,l') local_bounds) cycle;
      ...
      let compute_new_bound =
        let bound = Timeout.timed_run 5. (fun () ->
          ...
          let compute_termination (cycle, loops) =
            ...
            List.iter (fun (l,t,l') -> TimeBoundTable.add termination_table (l,orig t,l') local_bounds) cycle;
          ...
          SimpleCycle.find_loops ~relax_loops:conf.relax_loops trans_table (heuristic_for_cycle conf.transformation_type) appr program scc (l,t,l')
