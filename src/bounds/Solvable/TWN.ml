    ...
    (* list of all transitions in a cycle, twnloop doesn't require each transition to be in twn form, but together they do *)
      ...
      if (List.for_all (fun (t, entry) ->
        ...
        if List.for_all Option.is_some non_increasing_opt then
          lift_option @@ List.map (Transformation.transform transformation_type) (List.combine (List.map Option.get non_increasing_opt) twn_loops |> List.combine entries |> List.map (fun (x,(y,z)) -> (x,y,z))) (*tries to transform into loops. *)
        else
          None)
      else
      ...
      Timeout.timed_run 5. (fun () -> try
        ...
        let x =
          if Location.equal l l' then
            let f (l1,loop,l1') = Location.equal l l1 && Location.equal l' l1' && VarMap.equal Polynomial.equal (TransitionLabel.update_map transition) (TWNLoop.update_map loop) in
