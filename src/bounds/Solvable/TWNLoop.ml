...
module Make(PM: ClassicalProgramModules) = struct
  open PM
  ...
  let corresponding_transition t (trans: TransitionSet.t) =
    List.find
      (fun (_,t',_) -> TransitionLabel.id t == TransitionLabel.id t')
      (TransitionSet.to_list trans)

  let subsumed_transitions trans twn =
    List.map (fun t -> corresponding_transition t trans) twn.subsumed_transitionlabels

  let mk_transitions xs =
    let invariants = List.map TransitionLabel.invariant xs in
    let common_invariants =
      List.flatten invariants
      |> List.filter (fun atom -> List.for_all (List.exists (Atom.equal atom)) invariants)
    in
    {
        id = unique();
