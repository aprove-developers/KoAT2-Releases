...
let lift twn_proofs appr t var = function
  ...
  | Some xs -> xs
               ...
               |> Bound.sum
               |> tap (fun b -> ProofOutput.LocalProofOutput.add_to_proof twn_proofs  FormattedString.(fun () -> mk_str_header_small @@ "TWN Size Bound - Lifting for " ^ (Transition.to_id_string_pretty t) ^ " and " ^ Var.to_string ~pretty:true var ^ ": " ^ Bound.to_string ~pretty:true b))
...
let compute_time_bound twn_proofs loop =
  ...
  if Bound.is_infinity mprf_bound && Check_TWN.check_twn loop then
    TWN_Complexity.complexity twn_proofs ~termination:false loop (* We assume that loop terminates (since t terminates). *)
  else
    mprf_bound
      ...
      if SizeBoundTable.mem size_bound_table (t,var) && not @@ Bound.is_polynomial @@ Approximation.sizebound appr t var then
        ...
        if not @@ Bound.is_polynomial @@ Approximation.sizebound appr t var then (
          ProofOutput.LocalProofOutput.add_to_proof twn_proofs
            (fun () -> FormattedString.mk_str_header_big @@ "TWN. Size Bound: " ^ (Transition.to_id_string_pretty t) ^ " for " ^ Var.to_string ~pretty:true var);
          let loops_opt = SimpleCycle.find_loop twn_proofs heuristic_for_cycle ~relevant_vars:(Option.some @@ VarSet.singleton var) appr program trans t in
          if Option.is_some loops_opt then
            ...
            let local_bound =
                ...
                if List.is_empty order then None
                  ...
                  ProofOutput.LocalProofOutput.add_to_proof twn_proofs FormattedString.(fun () -> mk_str_line @@ "loop: " ^ Loop.to_string loop_red <> mk_str_line @@ "closed-form: " ^ PE.to_string_pretty closed_form <> mk_str_line @@ "runtime bound: " ^ Bound.to_string ~pretty:true time_bound);
