...
module Make (PM : ProgramTypes.ClassicalProgramModules) = struct
  ...
  let vars t =
    ...
    |> List.map Base.Set.to_list |> List.flatten |> VarSet.of_list
  ...
  let append ((guard, update) : t) ((guard', update') : t) =
    ...
    let new_update = Base.Map.map ~f:(Polynomial.substitute_f (substitution update)) update'
    and new_guard =
  ...
  let eliminate_non_contributors ?(relevant_vars = None) (loop : t) =
    let f loop non_contributors =
      (guard loop, Base.Set.fold ~f:Base.Map.remove non_contributors ~init:(update loop))
    in
