...
module Make(PM: ProgramTypes.ClassicalProgramModules): sig
  ...
  val complexity : ProofOutput.LocalProofOutput.t -> ?entry:(PM.Transition.t) option -> ?termination:bool -> Loop.t -> Bound.t

  (** Gets a {[TransitionLabel]} in twn-form and returns a complexity bound for the respective twn-loop. *)
  val complexity_ : ProofOutput.LocalProofOutput.t -> PM.Transition.t -> Bound.t
end
