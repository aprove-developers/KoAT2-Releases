...
val improve : rvg_with_sccs -> ?mprf_max_depth:int -> preprocess:(Program.t -> Program.t) -> local:([`MPRF | `TWN ] list) -> cfr:([`PartialEvaluation | `Chaining ] list) -> [ `Cost | `Time ] -> Program.t -> Approximation.t ->  Program.t * Approximation.t
