...
let lwt_parallel ~local (scc: TransitionSet.t) measure program max_depth appr =
  ...
  else if List.length local == 1 && (List.mem `TWN local || List.mem `TWNTransformGeneral local || List.mem `TWNTransformJordan local || List.mem `TWNTransform local) then
    improve_with_twn program scc measure (List.first local) appr 
  else if List.length local == 2 && (List.mem `MPRF local) && (List.mem `TWN local || List.mem `TWNTransformGeneral local || List.mem `TWNTransformJordan local || List.mem `TWNTransform local)  then 
