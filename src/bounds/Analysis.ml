...
let lwt_parallel ~local (scc: TransitionSet.t) measure program max_depth appr =
  ...
  else if List.mem `MPRF local |> not && (List.mem `TWN local || List.mem `TWNTransformGeneral local || List.mem `TWNTransformJordan local || List.mem `TWNTransform local) && (List.length local == 1) then
