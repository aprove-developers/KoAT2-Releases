...
module Make(A : BoundTypes.Approximation) =
  struct
    ...
    module VariableGraph_ = Approximation_.TransitionGraph_.VariableGraph_
    ...
    let pre (transitiongraph: TransitionGraph_.t)
            (l: TransitionGraph_.Graph.V.t)
        : TransitionGraph_.Graph.E.t list =
      TransitionGraph_.(Graph.pred_e (graph transitiongraph) l)
    ...
    let improve_nontrivial_scc (transitiongraph: TransitionGraph_.t)
                               (variablegraph: VariableGraph_.t)
                               (appr: Approximation_.t)
                               (scc: VariableGraph_.Graph.V.t list)
        : Approximation_.t =
      raise (Failure "Not yet implemented")
    ...
    let improve transitiongraph appr =
      let module C = Graph.Components.Make(VariableGraph_.Graph) in
      let variablegraph = TransitionGraph_.create_variable_graph transitiongraph in
      List.fold_left (fun appr scc -> improve_scc transitiongraph variablegraph appr scc) appr (C.scc_list (VariableGraph_.graph variablegraph))
