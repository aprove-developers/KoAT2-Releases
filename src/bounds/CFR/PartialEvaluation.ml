...
let minimalDisjointSCCs (original_sccs : TransitionSet.t list) =
  ...
  SCCSet.filter
    (fun scc1 ->
      not
        (SCCSet.exists (fun scc2 -> Base.Set.is_subset scc1 ~of_:scc2 && not (Base.Set.equal scc1 scc2)) tmp))
    tmp
         ...
         let all_l' =
           find_matching_locations location
             (Base.Set.filter
                ...
                transitions)
         in
         all_l' |> Base.Set.fold ~f:(fun tset l' -> rename_matching_trans location l' tset) ~init:merged_trans)
       ~init:transitions
