...
module CFR (Bound : BoundType.Bound) = struct
  ...
  let merge_appr (program : Program.t) (program_cfr : Program.t) appr =
    ...
    let appr = Approximation.empty |> TrivialTimeBounds.compute program_cfr in
    unchanged_trans
    |> Base.Set.fold
         ~f:(fun appr_cfr trans ->
           let timebound = appr |> flip Approximation.timebound trans
           and costbound = appr |> flip Approximation.costbound trans in
           (* Base.Set.fold
              ~f:(fun appr_cfr x ->
                let sizebound_x = Approximation.sizebound appr trans x in
                appr_cfr |> Approximation.add_sizebound sizebound_x trans x)
              (Program.vars program) ~init:appr_cfr *)
           appr_cfr
