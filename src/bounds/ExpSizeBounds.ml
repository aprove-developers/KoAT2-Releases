...
let improve_scc simplify_smt elcb_cache program ervg appr = function
  (* trivial SCCs of the RVG (without self-loop) *)
  | [((gt,l),v)] when not (ERVG.mem_edge ervg ((gt,l),v) ((gt,l),v)) ->
     let new_bound = ExpTrivialSizeBounds.compute elcb_cache program
  ...
  | scc ->
     let new_bound = ExpNontrivialSizeBounds.compute elcb_cache program (Approximation.timebound_gt appr) (Approximation.exptimebound appr)
...
let improve simplify_smt elcb_cache ervg sccs program appr =
  let execute () =
    List.fold_left (fun appr scc -> improve_scc simplify_smt elcb_cache program ervg appr scc) appr sccs
