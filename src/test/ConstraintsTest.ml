        ...
        "Constraints" >:::[
            ...
            ("get_coefficient_vector" >:::
                List.map (fun (expected, var, constr) ->
                      constr >:: (fun _ -> assert_equal ~cmp:list_equality ~printer:list_print (expected) (C.get_coefficient_vector (Polynomial.Monomial_.Var.of_string var) (Reader.read_constraint constr) )))
                        [
