...
module MakeOverIndeterminate (I : PolyTypes.Indeterminate) (Value : PolyTypes.Ring) = struct
  ...
  type t = { coeff : Value.t; mon : Monomial.t } [@@deriving eq, ord]

  let is_integral t = Value.is_integral t.coeff && Monomial.is_integral t.mon
  ...
  let to_string ?(to_file = false) ?(pretty = false) scaled =
    if Value.(scaled.coeff =~= one) then
      Monomial.to_string ~to_file ~pretty scaled.mon
    else if Value.(scaled.coeff =~= zero) then
      ""
    else
      Value.to_string scaled.coeff
      ^
      if Monomial.(scaled.mon =~= Monomial.one) then
        ""
      else
        (if pretty then
           "⋅"
  ...
  module BasePartialOrderImpl : PolyTypes.BasePartialOrder with type t = outer_t = struct
    ...
    let ( > ) s1 s2 =
      ...
      | s1, s2 ->
          if Monomial.(monomial s1 =~= monomial s2) && Value.compare (coeff s1) (coeff s2) > 0 then
            Some true
          else
            None
  end
  ...
  let eval_f scaled f = Value.mul scaled.coeff (Monomial.eval_f scaled.mon f)
  let eval scaled valuation = Value.mul scaled.coeff (Monomial.eval scaled.mon valuation)
  let mult_with_const const scaled = { scaled with coeff = Value.mul scaled.coeff const }

  let mul scaled1 scaled2 =
    { coeff = Value.mul scaled1.coeff scaled2.coeff; mon = Monomial.mul scaled1.mon scaled2.mon }


  let one = { coeff = Value.one; mon = Monomial.one }
  let lift mon = { coeff = Value.one; mon }
