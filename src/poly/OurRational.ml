...
let reduce_sign (n, d) =
  let sign =
    if (OurInt.is_negative n && OurInt.is_negative d) || ((not (OurInt.is_negative n)) && OurInt.is_negative d)
    then
      OurInt.minus_one
    else
      OurInt.one
...
let of_int i = (OurInt.of_int i, OurInt.one)

let of_int_tuple (n, d) =
  if d == 0 then
    raise (Div_Zero "OurRational.of_ourint div_zero")
  else
    (OurInt.of_int n, OurInt.of_int d) |> reduce
...
let of_ourint_tuple (n, d) =
  if OurInt.is_zero d then
    raise (Div_Zero "OurRational.of_ourint div_zero")
  else
    (n, d) |> reduce
...
let to_string (n, d) =
  if OurInt.(equal one d) then
    OurInt.to_string n
  else if OurInt.is_zero n then
    "0"
  else
    OurInt.to_string n ^ "/" ^ OurInt.to_string d
...
let add (n1, d1) (n2, d2) =
  if OurInt.is_zero n1 then
    (n2, d2)
  else if OurInt.is_zero n2 then
    (n1, d1)
  else
...
let is_negative (n, d) =
  (OurInt.is_negative n && not (OurInt.is_negative d)) || (OurInt.is_negative d && not (OurInt.is_negative n))


let compare r1 r2 =
  let diff = sub r1 r2 in
  if is_negative diff then
    -1
  else if is_zero diff then
    0
  else
    1


let is_ge a b = compare a b >= 0

let reciprocal (n, d) =
  if OurInt.is_zero n then
    raise (Div_Zero "OurRational.reciprocal div_zero")
  else
    (d, n)
...
let div r1 r2 =
  if is_zero r2 then
    raise (Div_Zero "OurRational.div div_zero")
  else
    mul r1 (reciprocal r2)
  ...
  if is_zero i && not (OurInt.is_zero n) then
    ...
    let rec helper i m =
      if OurInt.is_zero m then
        ...
        mul i (helper i OurInt.(m - one))
    in
    if OurInt.is_negative n then
      helper i (OurInt.abs n) |> reduce_sign |> reciprocal
    else
      helper i n |> reduce_sign
