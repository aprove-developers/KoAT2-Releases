...
module MakeOverIndeterminate(I : PolyTypes.Indeterminate)(Value : PolyTypes.Ring) =
  struct
    ...
    let of_sequence seq =
      let addEntry map (var, n) = Map.change map var ~f:(Option.some % Option.value_map ~f:((+)n) ~default:n) in
    ...
    let degree mon =
      Map.fold ~f:(fun ~key ~data a -> data + a) mon ~init:0

    let degree_variable var mon =
      Option.value (Map.find mon var) ~default:0
    ...
    let to_string ?(to_file = false) ?(pretty = false) mon =
      if Map.is_empty mon then
        ...
        in
        Map.to_alist mon
        |> List.map ~f:(uncurry entry_string)
        |> String.concat ~sep:"*"
    ...
    let rename (m: RenameMap.t) (mon: t) =
      Map.to_sequence mon
      |> Sequence.map ~f:(Tuple2.map1 (I.rename m))
      |> M.of_sequence_exn

    let indeterminates (t:t) =
      Set.stable_dedup_list (module I) @@ Map.keys t
    ...
    let mul =
      ...
      Map.merge ~f:(fun ~key me -> Some (let (p1,p2) = Map.Merge_element.(left me, right me) in addPowers ?p1 ?p2 ()))

    let pow t e =
      Map.map ~f:(fun i -> i*e) t

    let eval_f mon f =
      Map.fold ~f:(fun ~key ~data result -> Value.mul result (f key)) mon ~init:Value.one
