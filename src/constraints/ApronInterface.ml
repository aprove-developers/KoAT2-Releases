...
module Koat2Apron = struct
  (** Converts a koat variable to its apron equivalent. *)
  let var_to_apron : Var.t -> Apron.Var.t = Apron.Var.of_string % Var.to_string

  (** Converts a koat variable set to its apron equivalent. *)
  let vars_to_apron : VarSet.t -> Apron.Var.t array = VarSet.map_to_array var_to_apron
  ...
  let atom_to_apron (environment : Apron.Environment.t) (atom : Atom.t) : Apron.Tcons1.t =
    let open Apron in
    Atom.fold
      ~subject:(poly_to_apron environment) (* We have a<=b, Apron uses form c>=0, therefore we need 0<=b-a *)
      ~le:(fun expr1 expr2 ->
        Tcons1.make (Texpr1.binop Texpr1.Sub expr2 expr1 Texpr1.Int Texpr1.Up) Tcons1.SUPEQ)
      ~lt:(fun expr1 expr2 ->
        Tcons1.make (Texpr1.binop Texpr1.Sub expr2 expr1 Texpr1.Int Texpr1.Up) Tcons1.SUP)
      atom
  ...
  let constraint_to_apron (environment : Apron.Environment.t) (constr : Constraint.t) : Apron.Tcons1.earray =
    ...
    let single_constraints =
      constr |> Constraint.atom_list
      |> List.map ~f:(atom_to_apron environment)
      |> Array.of_list (* Pretty dump, but more readable *)
    in
    let constraints = Tcons1.array_make environment (Array.length single_constraints) in
...
module Apron2Koat = struct
  (** Converts an apron variable to its koat equivalent. *)
  let var_from_apron : Apron.Var.t -> Var.t = Var.of_string % Apron.Var.to_string
  ...
  let poly_from_apron (expr : Apron.Texpr1.t) : Polynomial.t =
    ...
    let rec expr_to_poly = function
      ...
      | Texpr1.Binop (Texpr1.Add, expr1, expr2, _, _) ->
          Polynomial.add (expr_to_poly expr1) (expr_to_poly expr2)
      | Texpr1.Binop (Texpr1.Sub, expr1, expr2, _, _) ->
          Polynomial.sub (expr_to_poly expr1) (expr_to_poly expr2)
      | Texpr1.Binop (Texpr1.Mul, expr1, expr2, _, _) ->
          Polynomial.mul (expr_to_poly expr1) (expr_to_poly expr2)
      | Texpr1.Binop (op, expr1, expr2, typ, _) ->
          raise
            (Failure
               ("Usage of not supported apron binary operator " ^ Texpr0.string_of_binop op ^ " with types "
              ^ Texpr0.string_of_typ typ ^ "!"))
    in
    expr |> Apron.Texpr1.to_expr |> expr_to_poly
  ...
  let atom_from_apron (single_constraint : Apron.Tcons1.t) : Constraint.t =
    let open Apron in
    let comparator =
      match Tcons1.get_typ single_constraint with
...
let abstract_to_string a =
  Apron.Abstract1.print Format.str_formatter a;
