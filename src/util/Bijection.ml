...
let amap (am, _) = am
let bmap (_, bm) = bm
let to_sequence t = Map.to_sequence (amap t)
let to_sequence_rev t = Map.to_sequence (bmap t)
let finda t a = Map.find (amap t) a
let findb t b = Map.find (bmap t) b
let find = finda
let finda_exn t a = Map.find_exn (amap t) a
let findb_exn t b = Map.find_exn (bmap t) b
let find_exn = finda_exn
let keys (type a b a_cmp b_cmp) (t : (a, b, a_cmp, b_cmp) t) = Map.keys (amap t)
let values (type a b a_cmp b_cmp) (t : (a, b, a_cmp, b_cmp) t) = Map.keys (bmap t)
...
let remove = removea
let add a b (amap, bmap) = (Map.add_exn ~key:a ~data:b amap, Map.add_exn ~key:b ~data:a bmap)
