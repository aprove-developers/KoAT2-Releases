  ...
  Polynomial.fold
    ...
    ~pow:(fun b e ->
      if e = 1 then
        ...
        Z3.Arithmetic.mk_mul context (List.init e ~f:(const b))
      else
        failwith "Polynomial exponents should be between 1 and n")
    ...
    RationalBound.fold_bound
      ...
      ~var:(fun var ->
        ...
        Z3.Boolean.mk_ite context
          (Z3.Arithmetic.mk_gt context varexp
             (Z3.Arithmetic.Integer.mk_numeral_s context @@ OurInt.to_string OurInt.zero))
          varexp
...
let from_formula context =
  Formula.fold ~subject:(from_poly context) ~le:(Z3.Arithmetic.mk_le context)
...
module Z3Solver = struct
  ...
  let ctx = Z3.mk_context [ ("model", "false"); ("proof", "false") ] |> Atomically.create

  let with_context (f : Z3.context -> 'a) : 'a = Atomically.run_atomically ctx f
  ...
  let result_is expected_result formula =
    ...
    if result == Z3.Solver.UNKNOWN then
      raise
        (SMTFailure
           ("SMT-Solver does not know a solution due to: " ^ Z3.Solver.get_reason_unknown optimisation_goal))
    else
  ...
  let satisfiable formula = result_is Z3.Solver.SATISFIABLE formula

  let unsatisfiable formula = result_is Z3.Solver.UNSATISFIABLE formula
  let tautology = unsatisfiable % Formula.neg

  let equivalent formula1 formula2 =
    (* Negating formula1 <=> formula2 *)
    Formula.Infix.((formula1 && Formula.neg formula2) || (formula2 && Formula.neg formula1)) |> unsatisfiable
  ...
  let get_model ?(coeffs_to_minimise = []) formula =
    ...
    if status == Z3.Solver.SATISFIABLE then
      optimisation_goal |> Z3.Optimize.get_model
      |> Option.map ~f:(fun model ->
             model |> Z3.Model.get_const_decls
             |> List.map ~f:(fun func_decl ->
                    let var = func_decl |> Z3.FuncDecl.get_name |> Z3.Symbol.get_string |> Var.of_string in
                    let value =
                      func_decl |> Z3.Model.get_const_interp model
                    ...
                    in
                    (var, value)))
      |? [] |> Valuation.from |> Option.some
    else
      None
end

module Z3SolverTimeout = struct
  module Valuation = Valuation.Make (OurInt)
             ...
             |> List.map ~f:(fun func_decl ->
                    ...
                    let value =
                      ...
                      if Z3.Arithmetic.is_int expr then
                        ...
                        expr |> Z3.Arithmetic.Real.get_ratio
                        (* TODO Round shouldnt be the solution, but do we need this anyway, since we ignore the values of helper variables? *)
                        |> Q.to_bigint
                    in
                    (var, value)))
...
module IncrementalZ3Solver = struct
  ...
  let pop t = Z3.Solver.pop (solver t) 1

  let result_is expected (solver, _) =
    Z3.Solver.check solver [] |> fun result ->
    if result == Z3.Solver.UNKNOWN then
      raise
        (SMTFailure ("SMT-Solver does not know a solution due to: " ^ Z3.Solver.get_reason_unknown solver))
    else
      result == expected


  let satisfiable = result_is Z3.Solver.SATISFIABLE
  let unsatisfiable = result_is Z3.Solver.UNSATISFIABLE
  let add (solver, context) formula = Z3.Solver.add solver [ from_formula context formula ]
  let add_real (solver, context) formula = Z3.Solver.add solver [ from_real_formula context formula ]

  let add_bound_comparison (solver, ctx) cmpoperator b1 b2 =
    let z3_compoperator =
      match cmpoperator with
    ...
    if result_is Z3.Solver.SATISFIABLE (slv, ctx) then
      let extract_from_model model =
        ...
        |> List.map ~f:(fun func_decl ->
               let var = func_decl |> Z3.FuncDecl.get_name |> Z3.Symbol.get_string |> Var.of_string in
               let value =
                 func_decl |> Z3.Model.get_const_interp model |> Option.value_exn (* Should be fine here *)
                 |> get_z3_value
               in
               (var, value))
      in
      Z3.Solver.get_model slv |> Option.map ~f:extract_from_model
    else
      None
  ...
  let get_expr_value ~val_of_z3int ~val_of_ratio expr =
    if Z3.Arithmetic.is_int expr then
      expr |> Z3.Arithmetic.Integer.get_big_int |> val_of_z3int
    else
      expr |> Z3.Arithmetic.Real.get_ratio
      (* TODO Round shouldnt be the solution, but do we need this anyway, since we ignore the values of helper variables? *)
      |> val_of_ratio
